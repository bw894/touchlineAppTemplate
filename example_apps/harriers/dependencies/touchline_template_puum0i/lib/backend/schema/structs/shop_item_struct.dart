...
class ShopItemStruct extends FFFirebaseStruct {
  ShopItemStruct({
    String? brandImage,
    String? brandName,
    String? description,
...
ShopItemStruct? updateShopItemStruct(
  ShopItemStruct? shopItem, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    shopItem
...
void addShopItemStructData(
  ...
  if (shopItem.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && shopItem.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final shopItemData = getShopItemFirestoreData(shopItem, forFieldValue);
  final nestedData = shopItemData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields = shopItem.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getShopItemFirestoreData(
  ShopItemStruct? shopItem, [
  bool forFieldValue = false,
]) {
  if (shopItem == null) {
  ...
  final firestoreData = mapToFirestore(shopItem.toMap());

  // Add any Firestore field values
  shopItem.firestoreUtilData.fieldValues
      .forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getShopItemListFirestoreData(
  List<ShopItemStruct>? shopItems,
) =>
    shopItems?.map((e) => getShopItemFirestoreData(e, true)).toList() ?? [];
