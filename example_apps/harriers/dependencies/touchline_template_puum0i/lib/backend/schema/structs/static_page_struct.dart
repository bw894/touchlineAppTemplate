...
class StaticPageStruct extends FFFirebaseStruct {
  StaticPageStruct({
    String? title,
...
StaticPageStruct? updateStaticPageStruct(
  StaticPageStruct? staticPage, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    staticPage
...
void addStaticPageStructData(
  ...
  if (staticPage.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && staticPage.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final staticPageData = getStaticPageFirestoreData(staticPage, forFieldValue);
  final nestedData = staticPageData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields = staticPage.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getStaticPageFirestoreData(
  StaticPageStruct? staticPage, [
  bool forFieldValue = false,
]) {
  if (staticPage == null) {
  ...
  final firestoreData = mapToFirestore(staticPage.toMap());

  // Add any Firestore field values
  staticPage.firestoreUtilData.fieldValues
      .forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getStaticPageListFirestoreData(
  List<StaticPageStruct>? staticPages,
) =>
    staticPages?.map((e) => getStaticPageFirestoreData(e, true)).toList() ?? [];
