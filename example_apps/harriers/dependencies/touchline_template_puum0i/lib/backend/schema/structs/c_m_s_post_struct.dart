...
class CMSPostStruct extends FFFirebaseStruct {
  CMSPostStruct({
    bool? enableComments,
    int? created,
    int? viewsNumber,
...
CMSPostStruct? updateCMSPostStruct(
  CMSPostStruct? cMSPost, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    cMSPost
...
void addCMSPostStructData(
  ...
  if (cMSPost.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && cMSPost.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final cMSPostData = getCMSPostFirestoreData(cMSPost, forFieldValue);
  final nestedData = cMSPostData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields = cMSPost.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getCMSPostFirestoreData(
  CMSPostStruct? cMSPost, [
  bool forFieldValue = false,
]) {
  if (cMSPost == null) {
  ...
  final firestoreData = mapToFirestore(cMSPost.toMap());

  // Add any Firestore field values
  cMSPost.firestoreUtilData.fieldValues.forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getCMSPostListFirestoreData(
  List<CMSPostStruct>? cMSPosts,
) =>
    cMSPosts?.map((e) => getCMSPostFirestoreData(e, true)).toList() ?? [];
