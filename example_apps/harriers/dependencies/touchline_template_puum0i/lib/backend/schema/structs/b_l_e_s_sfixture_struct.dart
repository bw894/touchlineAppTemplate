...
class BLESSfixtureStruct extends FFFirebaseStruct {
  BLESSfixtureStruct({
    String? summary,
    String? comp,
    String? awayBadge,
...
BLESSfixtureStruct createBLESSfixtureStruct({
  ...
  bool delete = false,
    ...
    BLESSfixtureStruct(
      ...
      team: team ?? (clearUnsetFields ? BLTeamStruct() : null),
...
BLESSfixtureStruct? updateBLESSfixtureStruct(
  BLESSfixtureStruct? bLESSfixture, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    bLESSfixture
...
void addBLESSfixtureStructData(
  ...
  if (bLESSfixture == null) {
    return;
  }
  if (bLESSfixture.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && bLESSfixture.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final bLESSfixtureData =
      getBLESSfixtureFirestoreData(bLESSfixture, forFieldValue);
  final nestedData =
      bLESSfixtureData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields = bLESSfixture.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getBLESSfixtureFirestoreData(
  BLESSfixtureStruct? bLESSfixture, [
  bool forFieldValue = false,
]) {
  if (bLESSfixture == null) {
  ...
  final firestoreData = mapToFirestore(bLESSfixture.toMap());

  // Handle nested data for "team" field.
  addBLTeamStructData(
    firestoreData,
    bLESSfixture.hasTeam() ? bLESSfixture.team : null,
    'team',
    forFieldValue,
  );

  // Add any Firestore field values
  bLESSfixture.firestoreUtilData.fieldValues
      .forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getBLESSfixtureListFirestoreData(
  List<BLESSfixtureStruct>? bLESSfixtures,
) =>
    bLESSfixtures?.map((e) => getBLESSfixtureFirestoreData(e, true)).toList() ??
    [];
