...
class ShopSizesStruct extends FFFirebaseStruct {
  ShopSizesStruct({
    bool? allowPreOrder,
    bool? beingSold,
    String? name,
...
ShopSizesStruct? updateShopSizesStruct(
  ShopSizesStruct? shopSizes, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    shopSizes
...
void addShopSizesStructData(
  ...
  if (shopSizes.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && shopSizes.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final shopSizesData = getShopSizesFirestoreData(shopSizes, forFieldValue);
  final nestedData = shopSizesData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields = shopSizes.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getShopSizesFirestoreData(
  ShopSizesStruct? shopSizes, [
  bool forFieldValue = false,
]) {
  if (shopSizes == null) {
  ...
  final firestoreData = mapToFirestore(shopSizes.toMap());

  // Add any Firestore field values
  shopSizes.firestoreUtilData.fieldValues
      .forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getShopSizesListFirestoreData(
  List<ShopSizesStruct>? shopSizess,
) =>
    shopSizess?.map((e) => getShopSizesFirestoreData(e, true)).toList() ?? [];
