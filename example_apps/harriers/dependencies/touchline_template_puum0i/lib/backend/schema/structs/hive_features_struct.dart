...
class HiveFeaturesStruct extends FFFirebaseStruct {
  HiveFeaturesStruct({
    bool? tickets,
...
HiveFeaturesStruct? updateHiveFeaturesStruct(
  HiveFeaturesStruct? hiveFeatures, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    hiveFeatures
...
void addHiveFeaturesStructData(
  ...
  if (hiveFeatures == null) {
    return;
  }
  if (hiveFeatures.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && hiveFeatures.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final hiveFeaturesData =
      getHiveFeaturesFirestoreData(hiveFeatures, forFieldValue);
  final nestedData =
      hiveFeaturesData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields = hiveFeatures.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getHiveFeaturesFirestoreData(
  HiveFeaturesStruct? hiveFeatures, [
  bool forFieldValue = false,
]) {
  if (hiveFeatures == null) {
  ...
  final firestoreData = mapToFirestore(hiveFeatures.toMap());

  // Add any Firestore field values
  hiveFeatures.firestoreUtilData.fieldValues
      .forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getHiveFeaturesListFirestoreData(
  List<HiveFeaturesStruct>? hiveFeaturess,
) =>
    hiveFeaturess?.map((e) => getHiveFeaturesFirestoreData(e, true)).toList() ??
    [];
