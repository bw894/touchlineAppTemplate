...
class CartLinesStruct extends FFFirebaseStruct {
  CartLinesStruct({
    int? qty,
    String? title,
    String? type,
...
CartLinesStruct? updateCartLinesStruct(
  CartLinesStruct? cartLines, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    cartLines
...
void addCartLinesStructData(
  ...
  if (cartLines.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && cartLines.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final cartLinesData = getCartLinesFirestoreData(cartLines, forFieldValue);
  final nestedData = cartLinesData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields = cartLines.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getCartLinesFirestoreData(
  CartLinesStruct? cartLines, [
  bool forFieldValue = false,
]) {
  if (cartLines == null) {
  ...
  final firestoreData = mapToFirestore(cartLines.toMap());

  // Add any Firestore field values
  cartLines.firestoreUtilData.fieldValues
      .forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getCartLinesListFirestoreData(
  List<CartLinesStruct>? cartLiness,
) =>
    cartLiness?.map((e) => getCartLinesFirestoreData(e, true)).toList() ?? [];
