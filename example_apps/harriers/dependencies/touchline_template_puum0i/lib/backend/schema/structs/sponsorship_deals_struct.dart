...
class SponsorshipDealsStruct extends FFFirebaseStruct {
...
SponsorshipDealsStruct? updateSponsorshipDealsStruct(
  SponsorshipDealsStruct? sponsorshipDeals, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    sponsorshipDeals
...
void addSponsorshipDealsStructData(
  ...
  if (sponsorshipDeals == null) {
    return;
  }
  if (sponsorshipDeals.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && sponsorshipDeals.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final sponsorshipDealsData =
      getSponsorshipDealsFirestoreData(sponsorshipDeals, forFieldValue);
  final nestedData =
      sponsorshipDealsData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields = sponsorshipDeals.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getSponsorshipDealsFirestoreData(
  SponsorshipDealsStruct? sponsorshipDeals, [
  bool forFieldValue = false,
]) {
  if (sponsorshipDeals == null) {
  ...
  final firestoreData = mapToFirestore(sponsorshipDeals.toMap());

  // Add any Firestore field values
  sponsorshipDeals.firestoreUtilData.fieldValues
      .forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getSponsorshipDealsListFirestoreData(
  List<SponsorshipDealsStruct>? sponsorshipDealss,
) =>
    sponsorshipDealss
        ?.map((e) => getSponsorshipDealsFirestoreData(e, true))
        .toList() ??
    [];
