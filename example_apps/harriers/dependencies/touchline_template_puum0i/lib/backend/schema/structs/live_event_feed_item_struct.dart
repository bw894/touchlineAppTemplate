...
class LiveEventFeedItemStruct extends FFFirebaseStruct {
...
LiveEventFeedItemStruct? updateLiveEventFeedItemStruct(
  LiveEventFeedItemStruct? liveEventFeedItem, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    liveEventFeedItem
...
void addLiveEventFeedItemStructData(
  ...
  if (liveEventFeedItem == null) {
    return;
  }
  if (liveEventFeedItem.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && liveEventFeedItem.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final liveEventFeedItemData =
      getLiveEventFeedItemFirestoreData(liveEventFeedItem, forFieldValue);
  final nestedData =
      liveEventFeedItemData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields = liveEventFeedItem.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getLiveEventFeedItemFirestoreData(
  LiveEventFeedItemStruct? liveEventFeedItem, [
  bool forFieldValue = false,
]) {
  if (liveEventFeedItem == null) {
  ...
  final firestoreData = mapToFirestore(liveEventFeedItem.toMap());

  // Add any Firestore field values
  liveEventFeedItem.firestoreUtilData.fieldValues
      .forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getLiveEventFeedItemListFirestoreData(
  List<LiveEventFeedItemStruct>? liveEventFeedItems,
) =>
    liveEventFeedItems
        ?.map((e) => getLiveEventFeedItemFirestoreData(e, true))
        .toList() ??
    [];
