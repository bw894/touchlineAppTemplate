...
class SponsorAdStruct extends FFFirebaseStruct {
  SponsorAdStruct({
    int? created,
    int? sortOrder,
    String? link,
...
SponsorAdStruct? updateSponsorAdStruct(
  SponsorAdStruct? sponsorAd, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    sponsorAd
...
void addSponsorAdStructData(
  ...
  if (sponsorAd.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && sponsorAd.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final sponsorAdData = getSponsorAdFirestoreData(sponsorAd, forFieldValue);
  final nestedData = sponsorAdData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields = sponsorAd.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getSponsorAdFirestoreData(
  SponsorAdStruct? sponsorAd, [
  bool forFieldValue = false,
]) {
  if (sponsorAd == null) {
  ...
  final firestoreData = mapToFirestore(sponsorAd.toMap());

  // Add any Firestore field values
  sponsorAd.firestoreUtilData.fieldValues
      .forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getSponsorAdListFirestoreData(
  List<SponsorAdStruct>? sponsorAds,
) =>
    sponsorAds?.map((e) => getSponsorAdFirestoreData(e, true)).toList() ?? [];
