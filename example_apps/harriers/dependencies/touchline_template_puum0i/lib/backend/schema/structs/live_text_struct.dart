...
class LiveTextStruct extends FFFirebaseStruct {
  LiveTextStruct({
...
LiveTextStruct? updateLiveTextStruct(
  LiveTextStruct? liveText, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    liveText
...
void addLiveTextStructData(
  ...
  if (liveText.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && liveText.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final liveTextData = getLiveTextFirestoreData(liveText, forFieldValue);
  final nestedData = liveTextData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields = liveText.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getLiveTextFirestoreData(
  LiveTextStruct? liveText, [
  bool forFieldValue = false,
]) {
  if (liveText == null) {
  ...
  final firestoreData = mapToFirestore(liveText.toMap());

  // Handle nested data for "relatedMatch" field.
  addBLESSfixtureStructData(
    firestoreData,
    liveText.hasRelatedMatch() ? liveText.relatedMatch : null,
    'relatedMatch',
    forFieldValue,
  );

  // Add any Firestore field values
  liveText.firestoreUtilData.fieldValues
      .forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getLiveTextListFirestoreData(
  List<LiveTextStruct>? liveTexts,
) =>
    liveTexts?.map((e) => getLiveTextFirestoreData(e, true)).toList() ?? [];
