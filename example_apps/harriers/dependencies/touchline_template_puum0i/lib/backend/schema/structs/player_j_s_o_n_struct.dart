...
class PlayerJSONStruct extends FFFirebaseStruct {
  PlayerJSONStruct({
    String? playerFirst,
    String? playerLast,
    int? playerNumber,
...
PlayerJSONStruct? updatePlayerJSONStruct(
  PlayerJSONStruct? playerJSON, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    playerJSON
...
void addPlayerJSONStructData(
  ...
  if (playerJSON.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && playerJSON.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final playerJSONData = getPlayerJSONFirestoreData(playerJSON, forFieldValue);
  final nestedData = playerJSONData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields = playerJSON.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getPlayerJSONFirestoreData(
  PlayerJSONStruct? playerJSON, [
  bool forFieldValue = false,
]) {
  if (playerJSON == null) {
  ...
  final firestoreData = mapToFirestore(playerJSON.toMap());

  // Add any Firestore field values
  playerJSON.firestoreUtilData.fieldValues
      .forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getPlayerJSONListFirestoreData(
  List<PlayerJSONStruct>? playerJSONs,
) =>
    playerJSONs?.map((e) => getPlayerJSONFirestoreData(e, true)).toList() ?? [];
