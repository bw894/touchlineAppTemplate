...
class NewSingleProductWPStruct extends FFFirebaseStruct {
...
NewSingleProductWPStruct? updateNewSingleProductWPStruct(
  NewSingleProductWPStruct? newSingleProductWP, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    newSingleProductWP
...
void addNewSingleProductWPStructData(
  ...
  if (newSingleProductWP == null) {
    return;
  }
  if (newSingleProductWP.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && newSingleProductWP.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final newSingleProductWPData =
      getNewSingleProductWPFirestoreData(newSingleProductWP, forFieldValue);
  final nestedData =
      newSingleProductWPData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields =
      newSingleProductWP.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getNewSingleProductWPFirestoreData(
  NewSingleProductWPStruct? newSingleProductWP, [
  bool forFieldValue = false,
]) {
  if (newSingleProductWP == null) {
  ...
  final firestoreData = mapToFirestore(newSingleProductWP.toMap());

  // Add any Firestore field values
  newSingleProductWP.firestoreUtilData.fieldValues
      .forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getNewSingleProductWPListFirestoreData(
  List<NewSingleProductWPStruct>? newSingleProductWPs,
) =>
    newSingleProductWPs
        ?.map((e) => getNewSingleProductWPFirestoreData(e, true))
        .toList() ??
    [];
