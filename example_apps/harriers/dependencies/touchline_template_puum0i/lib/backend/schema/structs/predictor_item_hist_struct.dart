...
class PredictorItemHistStruct extends FFFirebaseStruct {
...
PredictorItemHistStruct createPredictorItemHistStruct({
  ...
  bool delete = false,
    ...
    PredictorItemHistStruct(
      ...
      match: match ?? (clearUnsetFields ? BLESSfixtureStruct() : null),
      created: created,
...
PredictorItemHistStruct? updatePredictorItemHistStruct(
  PredictorItemHistStruct? predictorItemHist, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    predictorItemHist
...
void addPredictorItemHistStructData(
  ...
  if (predictorItemHist == null) {
    return;
  }
  if (predictorItemHist.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && predictorItemHist.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final predictorItemHistData =
      getPredictorItemHistFirestoreData(predictorItemHist, forFieldValue);
  final nestedData =
      predictorItemHistData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields = predictorItemHist.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getPredictorItemHistFirestoreData(
  PredictorItemHistStruct? predictorItemHist, [
  bool forFieldValue = false,
]) {
  if (predictorItemHist == null) {
  ...
  final firestoreData = mapToFirestore(predictorItemHist.toMap());

  // Handle nested data for "match" field.
  addBLESSfixtureStructData(
    firestoreData,
    predictorItemHist.hasMatch() ? predictorItemHist.match : null,
    'match',
    forFieldValue,
  );

  // Add any Firestore field values
  predictorItemHist.firestoreUtilData.fieldValues
      .forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getPredictorItemHistListFirestoreData(
  List<PredictorItemHistStruct>? predictorItemHists,
) =>
    predictorItemHists
        ?.map((e) => getPredictorItemHistFirestoreData(e, true))
        .toList() ??
    [];
