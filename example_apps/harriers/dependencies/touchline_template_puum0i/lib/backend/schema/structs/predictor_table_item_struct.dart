...
class PredictorTableItemStruct extends FFFirebaseStruct {
...
PredictorTableItemStruct? updatePredictorTableItemStruct(
  PredictorTableItemStruct? predictorTableItem, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    predictorTableItem
...
void addPredictorTableItemStructData(
  ...
  if (predictorTableItem == null) {
    return;
  }
  if (predictorTableItem.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && predictorTableItem.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final predictorTableItemData =
      getPredictorTableItemFirestoreData(predictorTableItem, forFieldValue);
  final nestedData =
      predictorTableItemData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields =
      predictorTableItem.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getPredictorTableItemFirestoreData(
  PredictorTableItemStruct? predictorTableItem, [
  bool forFieldValue = false,
]) {
  if (predictorTableItem == null) {
  ...
  final firestoreData = mapToFirestore(predictorTableItem.toMap());

  // Add any Firestore field values
  predictorTableItem.firestoreUtilData.fieldValues
      .forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getPredictorTableItemListFirestoreData(
  List<PredictorTableItemStruct>? predictorTableItems,
) =>
    predictorTableItems
        ?.map((e) => getPredictorTableItemFirestoreData(e, true))
        .toList() ??
    [];
